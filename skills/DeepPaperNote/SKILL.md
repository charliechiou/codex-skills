...
- say `已儲存到 Obsidian` only when the write step has actually succeeded
...
- Do not use soft reasons such as keeping the note light, values already transcribed, future lookup, or convenient back-reference as the standalone `當前狀態` for a usable candidate.
- The note must pass a style gate: no mixed Chinese-English prose lines except stable proper nouns or citation metadata.
- The note must also pass a locale gate: prefer Taiwan terminology and orthography consistently, for example `資料集` over `資料集`, `訓練` over `訓練`, `論文` over `論文`, `圖表` over `圖表`, and `程式碼` over `程式碼` unless quoting source metadata.
- The style gate also rejects mechanical term-replacement artifacts such as `KV快取 of`, `批次ing`, `In相關 Researcher`, or `Single 序列 generation`; rewrite the sentence naturally instead of preserving a partially translated phrase.
...
- in `final_readability_review`, ordinary English phrase leftovers should usually be rewritten into natural Traditional Chinese with Taiwan wording, while stable proper nouns may remain in English
