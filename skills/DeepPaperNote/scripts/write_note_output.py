#!/usr/bin/env python3
"""Write the final Markdown note into the configured notes root or workspace output."""
...
def parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description=__doc__ or "write note output")
    ...
    p.add_argument("--subdir", default="", help="Configured-root-relative subdirectory.")
...
def main() -> None:
    ...
    if not title:
        raise SystemExit("write_note_output.py requires --title or metadata with a title.")
    ...
    if args.lint_json:
        ...
        if not lint.get("passes_basic_structure", False):
            raise SystemExit("write_note_output.py refused to write note because basic structure lint failed.")
        if not lint.get("passes_style_gate", False):
            raise SystemExit("write_note_output.py refused to write note because style gate failed.")
        if not lint.get("passes_math_gate", False):
            raise SystemExit("write_note_output.py refused to write note because math gate failed.")
        if "passes_figure_gate" in lint and not lint.get("passes_figure_gate", False):
            raise SystemExit("write_note_output.py refused to write note because figure gate failed.")
        if "passes_plan_gate" in lint and not lint.get("passes_plan_gate", False):
            raise SystemExit("write_note_output.py refused to write note because plan gate failed.")
        if "passes_substantive_content" in lint and not lint.get("passes_substantive_content", False):
            raise SystemExit("write_note_output.py refused to write note because substantive content gate failed.")
    ...
    if args.content_file:
        ...
        raise SystemExit("write_note_output.py requires --content-file, --content, or --stdin.")
    ...
    target_path = resolve_note_output_path(
        config,
        title=title,
    ...
    if config.get("notes_root") or config.get("obsidian_vault"):
        payload["notes_root"] = str(Path(config.get("notes_root") or config.get("obsidian_vault")).expanduser().resolve())
